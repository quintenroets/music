...
export default {
  ...
  methods: {
    ...
    setStreamingUrl: function () {
      EpisodeService.getStreamingUrl(this.episode).then((response) => {
        this.streamingUrl = response;
      });
    },
    setSubtitleUrl: function () {
      EpisodeService.getSubtitleUrl(this.episode).then((response) => {
        this.subtitleUrl = process.env.VUE_APP_API_URL + response;
      });
    },
